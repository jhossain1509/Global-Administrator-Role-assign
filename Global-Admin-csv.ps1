...
$userCsvPath = Read-Host "Enter user CSV path"
if (-not (Test-Path $userCsvPath)) {
    Write-Log "CSV not found:  $userCsvPath" -Level "ERROR"
    exit
}
...
foreach ($user in $userList) {
    ...
    Show-ProgressBar -Current $script:processedCount -Total $total -CurrentUser $user.username
    
    Assign-GlobalAdminToUser -UserPrincipalName $user.username -RoleId $role.Id
